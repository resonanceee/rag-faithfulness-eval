...
### Option A — npm (plugin spec in opencode config)
...
### Option B — local file copy (single-file entry)
...
3. **Active model detection** — captured from `chat.params` /
   ...
   `RFE_ACTIVE_MODEL`.
4. **Judge selection** — active model matching GLM-5.3-Flash (contains `glm-5`
   and `flash`, case-insensitive) → judge = DeepSeek-V4.1-Flash; otherwise
   judge = GLM-5.3-Flash. A model never judges itself.
5. **Per reply** — on assistant `message.updated` with `time.completed`, reply
   ...
   premises. Everything is async fire-and-forget; the reply is never blocked.
6. **Verdicts** — `faithful` passes silently. Any `unfaithful`/`unverifiable`
   ...
   (`client.session.prompt`, `noReply: true` so it never auto-triggers a new
   turn and can't loop). If injection fails, falls back to a `tui.toast.show`
   warning plus a structured log line. Claims are never auto-corrected.
...
8. **Caching + logging** — verdict key `sha256(model + "\x00" + context +
   ...
   `<RFE_CACHE_DIR>/opencode-cache-<model>.jsonl`; otherwise memory-only.
   Every judge API call emits a token log line (tokens only, no USD) to
   stderr or `RFE_JUDGE_LOG`.
...
| `RFE_JUDGE_GLM_MODEL`| `hf:zai-org/GLM-5.3-Flash` (synthetic) / `z-ai/glm-5.3-flash` (openrouter) | GLM judge model id |
| `RFE_JUDGE_DS_MODEL` | `hf:deepseek-ai/DeepSeek-V4.1-Flash` (synthetic) / `deepseek/deepseek-chat-v4.1-flash` (openrouter) | DeepSeek judge model id |
